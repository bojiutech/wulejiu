...
namespace HN 
{
			...
			if (rs)
			{
				...
				if (_array.IsArray())
				{
					...
					for (int i = 0; i < _array.Size(); ++i)
					{
						...
						product->SerialNumber = atoi(_array[i]["ID"].GetString());
						product->stype = atoi(_array[i]["stype"].GetString());
	...
	INT ProductManger::getProductTypeNum(INT type)
	{
		int temp = 0;
		for (auto ite = _products.begin(); ite != _products.end(); ite++)
		{
			PRODUCT_INFO* info = (PRODUCT_INFO*)(*ite);
			if (info->stype == type)
			{
				temp++;
			}
