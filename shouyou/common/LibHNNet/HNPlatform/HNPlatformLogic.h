...
namespace HN
{
	...
	public:
		...
		// 发送数据（带回掉, 返回消息辅助ID）
		void sendData(UINT MainID, UINT sendAssitantID, UINT responseAssitantID, void* object, INT objectSize, SEL_SocketMessage selector);
		...
		// 登陆
		void login(const std::string& name, const std::string& password, bool forced = false);

		// 注册
		//void regist(BYTE byFastRegister, UINT uNameID, const std::string& usn, const std::string& name = "", const std::string& password = "");

		// 注册
		void regist(BYTE byRegisteType, const std::string& usn = "", bool bBoy = true, const std::string& uionID = "", 
			const std::string& nickName = "", const std::string& name = "", const std::string& password = "", 
			const std::string& phoneNum = "", const std::string&  headUrl = "");
