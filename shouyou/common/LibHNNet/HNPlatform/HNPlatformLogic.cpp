...
namespace HN
{
	...
	// 发送数据（带辅助ID不同的回调）
	void HNPlatformLogic::sendData(UINT MainID, UINT sendAssitantID, UINT responseAssitantID, void* object, INT objectSize, SEL_SocketMessage selector)
	{
		_socketLogic->addEventSelector(MainID, responseAssitantID, selector);
		_socketLogic->send(MainID, sendAssitantID, HNSocketProtocolData::PlatformCheckCode, object, objectSize);
	}
	...
	void HNPlatformLogic::login(const std::string& name, const std::string& password, bool forced/* = false*/)
	{
	...
	*/
	...
	void HNPlatformLogic::regist(BYTE byRegisteType, const std::string& usn /*= ""*/, bool bBoy /*= true*/, 
		const std::string& uionID /*= ""*/, const std::string& nickName /*= ""*/, const std::string& name /*= ""*/, 
		const std::string& password /*= ""*/, const std::string& phoneNum /*= ""*/, const std::string& headUrl/*= ""*/)
	{
		MSG_GP_S_Register data = { 0 };
		data.byFromPhone = 1;
		data.byRegisteType = byRegisteType;
		data.bBoy = bBoy;
		strcpy(data.szUionID, uionID.c_str());
		strcpy(data.szName, name.c_str());
		strcpy(data.szPswd, password.c_str());
		strcpy(data.szHardID, usn.c_str());
		//strcpy(data.szPhoneNum, phoneNum.c_str());
		strcpy(data.szWeChatNickName, nickName.c_str());
		strcpy(data.headUrl, headUrl.c_str());
	...
	void HNPlatformLogic::onConnected(bool connect)
	{
		_connected = connect;
		dispatchMessage([connect](IPlatformMessageDelegate* delegate) -> bool
		{
			delegate->I_P_M_Connect(connect);
			return false;
		});
	}
