...
namespace HN
{
	...
	public:
		...
		// 添加用户信息
		const UserInfoStruct* addUser(UserInfoStruct* user);
		// 清空数据
		void clear();
	...
	public:
		// 通过用户ID查找用户
		const UserInfoStruct* findUser(INT userID);
		// 通过桌号，座位号查找用户
		const UserInfoStruct* findUser(BYTE deskNo, BYTE station);
		// 获取旁观玩家
	...
	protected:
		UserInfoStruct* getUser(INT userId);
