...
typedef struct tagMSG_GP_SR_Register
{
	...
	CHAR								szPswd[64];		// 密码
	UINT								LogonTimes;		// 登陆次数
	//CHAR								szPhoneNum[50];	// 手机号
	CHAR								szWeChatNickName[50];	// 微信昵称
	bool								bBoy;			// 性别
	CHAR								headUrl[256];	// 头像url
	CHAR								szUionID[64];	// 微信、QQ登陆标识
	CHAR								szAgentName[100];	// 代理商名称
	INT									iTuiGuangID;		//推广员ID
}  MSG_GP_S_Register;
...
/////////////////////////////////////////////VIP房间创建，进入/////////////////////////////////
//VIP桌子
#define MDM_GP_DESK_VIP					    136
#define ASS_GP_BUY_DESK						0				//购买桌子
#define ASS_GP_ENTER_DESK					1				//进入vip桌子	
#define ASS_GP_DESK_VIP_CONFIG				2				//请求购买桌子配置
#define ASS_GP_MONEY_TO_DIAMOND				3				//兑换钻石
...
{
	int iUserID;//用户ID
	int iResult;//购买结果(1:购买成功；2：用户不存在；3：钱包金币不足；4：购买金币不合法;5:桌子已购买完)
	int iRoomID;//房间号
	int iDeskID;//桌子号
...
struct MSG_GP_S_VIP_DESK_CONFIG
{
	int iUserID;//用户ID
	MSG_GP_S_VIP_DESK_CONFIG()
	{
		memset(this, 0, sizeof(MSG_GP_S_VIP_DESK_CONFIG));
	}
};
struct MSG_GP_S_VIP_DESK_CONFIG_RES
{
	int iUserID;//用户ID
	int iGameCount[5];//游戏局数
	int iJewels[5];//局数对应的钻石
	MSG_GP_S_VIP_DESK_CONFIG_RES()
	{
		memset(this, 0, sizeof(MSG_GP_S_VIP_DESK_CONFIG_RES));
	}
};
...
/////////////////////////////////////////////大厅获取断线重连协议/////////////////////////////////
//VIP桌子
#define MDM_GP_CUTNET_ROOMINFO					    139		//断线重连接口
#define ASS_GP_CUTNET_GET_ROOMINFO					0		//获取断线房间信息
...
//HandleCode: 0未找到断线信息，1,可购买房间获取信息成功，3可购买房间获取信息失败，4普通房间获取信息成功
