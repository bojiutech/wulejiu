...
/*********************************************************************************/
// 购买房间相关
#define MDM_GR_VIP_ROOM					181						//桌子解散主消息
#define ASS_GR_DESKRUNOUT				0						//解散桌子(已经解散或申请解散)
#define ASS_GR_DISSMISS_AGREE			1						//是否同意桌子解散
#define ASS_GR_DESK_INFO				2						//获取桌子信息
#define ASS_GR_DISSMISS_AGREE_RES		3						//是否同意桌子解散结果通知
...
// 桌子已到期
typedef struct VIP_Desk_RunOut
{
	bool			bSuccess;			//flase: 表示申请解散，true： 表示已经解散
	BYTE			bDeskIndex;			//桌子号码
	INT				iUserID;			//申请解散桌子用户ID
	VIP_Desk_RunOut()
	{
		bSuccess = false;
		bDeskIndex = 255;
...
// 是否同意申请解散桌子
typedef struct VIP_Desk_Dismiss_Notify
{
	int	iUserID;				///申请玩家ID
	VIP_Desk_Dismiss_Notify()
	{
		memset(this, 0, sizeof(VIP_Desk_Dismiss_Notify));
	}
} VIP_Desk_Dismiss_Notify;

// 是否同意申请解散桌子
typedef struct VIP_Desk_Dismiss_Agree
{
	bool			bAgree;				///是否同意
	VIP_Desk_Dismiss_Agree()
	{
		memset(this, 0, sizeof(VIP_Desk_Dismiss_Agree));
	}
} VIP_Desk_Dismiss_Agree;

// 是否同意申请解散桌子结果
typedef struct VIP_Desk_Dismiss_Agree_Res
{
	INT				iUserID;			///同意玩家ID
	bool			bAgree;				///是否同意
	VIP_Desk_Dismiss_Agree_Res()
	{
		memset(this, 0, sizeof(VIP_Desk_Dismiss_Agree_Res));
	}
} VIP_Desk_Dismiss_Agree_Res;

// 桌子信息
typedef struct Get_VIP_Desk_Info
{
	BYTE			byDeskIndex;				///桌号

	Get_VIP_Desk_Info()
	{
		memset(this, 0, sizeof(Get_VIP_Desk_Info));
	}
} Get_VIP_Desk_Info;


// 获取桌子信息结果
typedef struct VIP_Desk_Info_Result
{
	BYTE			byDeskIndex;				///桌号
	INT				iOwenerID;					//桌主ID
	...
	VIP_Desk_Info_Result()
	{
		memset(this, 0, sizeof(VIP_Desk_Info_Result));
	}
} VIP_Desk_Info_Result;

///剩余局数
struct LastRounds{
	int		iLastRounds;	//剩余局数
...
typedef struct tagMSG_GR_S_UserSit
{
	...
	CHAR								szCreateRoomPassword[20];			//创建房间桌子密码
}  MSG_GR_S_UserSit;
...
//////////////////////////////////////////语音功能//////////////////////////////////////
#define ASS_GR_VOIEC					200									///语音发送消息
#define RES_GR_VOIEC_SUC				1									///语音发送成功
#define RES_GR_VOIEC_ERR				2									///语音发送失败
