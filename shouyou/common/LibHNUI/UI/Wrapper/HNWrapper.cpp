...
namespace HN
{
	...
	Vec2 HNWrapper::convertToNodeSpace(const Vec2& worldPoint) const
	{
		return _component->convertToNodeSpace(worldPoint);
	}
	...
	void HNWrapper::setRotation(float rotation)
	{
		_component->setRotation(rotation);
	}
	...
	void HNWrapper::setLocalZOrder(int localZOrder)
	{
		_component->setLocalZOrder(localZOrder);
	}

	int HNWrapper::getLocalZOrder()
	{
		return _component->getLocalZOrder();
	}
