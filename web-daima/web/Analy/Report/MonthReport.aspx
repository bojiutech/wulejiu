<%@ Page Language="C#" AutoEventWireup="true" Codebehind="MonthReport.aspx.cs" Inherits="DataAnaly.Analy.Report.MonthReport" %>
		...
		<div id="content">
			...
			<div class="datalist">
				...
				<div class="search" style="width: auto;">
					日期：<input type="text" name="txtBeginDate" id="txtBeginDate" onfocus="WdatePicker({dateFmt:'yyyy-MM',maxDate:'#F{$dp.$D(\'txtEndDate\')}'});" />&nbsp;&nbsp;
					到<input type="text" name="txtEndDate" id="txtEndDate" onfocus="WdatePicker({minDate:'#F{$dp.$D(\'txtBeginDate\')}',dateFmt:'yyyy-MM'});" />&nbsp;&nbsp;<span
				...
				<table id="tbContent">
					...
					<tbody id="tbdDataList">
						<asp:Repeater runat="server" ID="rptDataList">
							<ItemTemplate>
								<tr style="cursor: auto;">
									...
									<td align="center">
										<a href="/Analy/Report/DateReport.aspx?bd=<%# Eval( "ReportDate","{0:yyyy-MM-dd}" )%>&ed=<%# Eval( "ReportDate","{0:yyyy-MM}" )+"-"+(DateTime.DaysInMonth(Convert.ToInt32( Eval( "ReportDate","{0:yyyy}" )),Convert.ToInt32(Eval( "ReportDate","{0:MM}" ))))%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看当月每日数据统计详细信息" class="hrefColor">
											<%# Eval( "ReportDate","{0:yyyy年MM月}" )%>
										</a>
									</td>
									<td align="center">
										<a href="/Analy/Report/AgencyUsers.aspx?bd=<%# Eval( "ReportDate","{0:yyyy-MM-dd}" )%>&ed=<%# Eval( "ReportDate","{0:yyyy-MM}" )+"-"+(DateTime.DaysInMonth(Convert.ToInt32( Eval( "ReportDate","{0:yyyy}" )),Convert.ToInt32(Eval( "ReportDate","{0:MM}" ))))%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看当月每日代理商直属玩家注册统计详细信息" class="hrefColor">
											<%# Eval( "RegNum" ).ToString()%>
										</a>
									</td>
									<td align="center">
										<a href="/Analy/Records/UserLoginList.aspx?bd=<%# Eval( "ReportDate","{0:yyyy-MM-dd}" )%>&ed=<%# Eval( "ReportDate","{0:yyyy-MM}" )+"-"+(DateTime.DaysInMonth(Convert.ToInt32( Eval( "ReportDate","{0:yyyy}" )),Convert.ToInt32(Eval( "ReportDate","{0:MM}" ))))%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看当月每日代理商直属玩家注册统计详细信息" class="hrefColor">
									...
									<td align="center">
										<a href="/Analy/Records/GameRecord.aspx?bd=<%# Eval( "ReportDate","{0:yyyy-MM-dd}" )%>&ed=<%# Eval( "ReportDate","{0:yyyy-MM}" )+"-"+(DateTime.DaysInMonth(Convert.ToInt32( Eval( "ReportDate","{0:yyyy}" )),Convert.ToInt32(Eval( "ReportDate","{0:MM}" ))))%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看当月每日抽水统计信息" class="hrefColor">
						...
						<asp:Literal runat="server" ID="ltNonData" Text="<tr><td colspan='16'>还没有任何数据。</td></tr>"></asp:Literal></tbody></table>
				<div class="pager">
