<%@ Page Language="C#" AutoEventWireup="true" Codebehind="DateReport.aspx.cs" Inherits="DataAnaly.Analy.Report.DateReport" %>
						...
						<asp:Repeater runat="server" ID="rptDataList">
							<ItemTemplate>
								<tr style="cursor: auto;">
									...
									<td align="center">
										<a href="/Analy/Records/UserInfoList.aspx?type=0&bd=<%# Eval( "Date","{0:yyyy-MM-dd}" )%>&ed=<%# Eval( "Date","{0:yyyy-MM-dd}" )%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看当日注册信息" class="hrefColor">
											<%# Eval( "RegNum" ).ToString()%>
										</a>
									</td>
									<td align="center">
										<a href="/Analy/Records/UserLoginList.aspx?bd=<%# Eval( "Date","{0:yyyy-MM-dd}" )%>&ed=<%# Eval( "Date","{0:yyyy-MM-dd}" )%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看当日玩家登录信息" class="hrefColor">
									...
									<td align="center">
										<a href="/Analy/Records/ActiveList.aspx?date=<%# Eval( "Date","{0:yyyy-MM-dd}" )%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看当日活跃用户列表" class="hrefColor">
											<%# Eval( "ActiveNum" ).ToString()%>
										</a>
									</td>
									<td align="center">
										<a href="/Analy/NewUser/UserLoginList.aspx?date=<%# Eval( "Date","{0:yyyy-MM-dd}" )%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看当日新进入玩家列表" class="hrefColor">
											<%# Eval( "NewUserNum" ).ToString()%>
										</a>
									</td>
									<td align="center">
										<a href="/Analy/Records/PayList.aspx?bd=<%# Eval( "Date","{0:yyyy-MM-dd}" )%>&ed=<%# Eval( "Date","{0:yyyy-MM-dd}" )%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看当日玩家充值记录" class="hrefColor">
									...
									<td align="center">
										<a href="/Analy/Records/GameRecord.aspx?bd=<%# Eval( "Date","{0:yyyy-MM-dd}" )%>&ed=<%# Eval( "Date","{0:yyyy-MM-dd}" )%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看当日游戏记录信息" class="hrefColor">
