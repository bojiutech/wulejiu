<%@ Page Language="C#" AutoEventWireup="true" Codebehind="UserLoginList.aspx.cs"
	Inherits="DataAnaly.Analy.NewUser.UserLoginList" %>
		...
		<div id="content">
			...
			<div class="datalist">
				...
				<div class="search" style="width: auto;">
					日期：<input type="text" name="txtDate" id="txtDate" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'});"
						onchange="OnSearch()" />&nbsp;&nbsp;<span onclick="Clear();">清空</span>
						...
						<asp:Repeater runat="server" ID="rptDataList">
							<ItemTemplate>
								<tr>
									...
									<td align="center">
										<a href="/Analy/Records/UserLoginDetail.aspx?bd=<%= HN.Common.CommonManager.Web.RequestUrlDecode( "date", "" )%>&ed=<%= HN.Common.CommonManager.Web.RequestUrlDecode( "date", "" )%>&un=<%# Eval( "UserName" ).ToString()%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看<%# Eval( "UserName" ).ToString()%>用户当天登录大厅详细信息" class="hrefColor">
									...
									<td align="center">
										<a href="/Analy/Records/GameRecord.aspx?bd=<%= HN.Common.CommonManager.Web.RequestUrlDecode( "date", "" )%>&ed=<%= HN.Common.CommonManager.Web.RequestUrlDecode( "date", "" )%>&un=<%# Eval( "UserName" ).ToString()%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看<%# Eval( "UserName" ).ToString()%>用户当天游戏记录" class="hrefColor">
											<%# Eval( "GameCount" )%>
										</a>
									</td>
									<td align="center">
										<a href="/Analy/Records/PayList.aspx?bd=<%= HN.Common.CommonManager.Web.RequestUrlDecode( "date", "" )%>&ed=<%= HN.Common.CommonManager.Web.RequestUrlDecode( "date", "" )%>&un=<%# Eval( "UserName" ).ToString()%>&reurl=<%=Server.UrlEncode( Request.Url.AbsoluteUri ) %>"
											title="点击可查看<%# Eval( "UserName" ).ToString()%>用户当天充值记录" class="hrefColor">
											<%# Eval( "TotalPayMoney" )%>
										</a>
									</td>
									<td align="center">
