...
<body>
				...
				<table width="100%" border="0" align="center" cellpadding="5" cellspacing="0">
					...
					<asp:Repeater runat="server" ID="rptMoneyList">
						<ItemTemplate>
							<tr>
								<td align="center">
									&nbsp;&nbsp;<%# "<a href='Pay.aspx?" + ( string.IsNullOrEmpty( typepay ) ? "" : "pay=" + typepay + "&" ) + "money=" + Eval( "Money" ).ToString() + (!string.IsNullOrEmpty(bd) && HN.Common.CommonManager.String.IsDateTime(bd) ? "&bd=" + bd : "") + (!string.IsNullOrEmpty(ed) && HN.Common.CommonManager.String.IsDateTime(ed) ? "&ed=" + ed : "") + (string.IsNullOrEmpty(un) ? "" : "&un=" + un) + "'>" + Eval( "Money" ).ToString() + "元面值</a>"%></td>
							</tr>
						</ItemTemplate>
					</asp:Repeater>
				</table>
					...
					<asp:Repeater ID="rptDataList" runat="server">
						...
						<ItemTemplate>
							<tr>
                                ...
                                <td>
									<%# type(Eval("Cardtype").ToString())%>
								</td>
								<td>
									<%# Eval("pay").ToString().ToLower() == "true" ? "<font color=\"red\">充 值</font>" : "未充值"%>
								</td>
								<td>
								&nbsp;&nbsp;<a href="PayDel.aspx?id=<%# Eval("ID") %>&url=<%=Server.UrlEncode(Request.Url.AbsoluteUri) %>" onclick="javascript:return confirm('您确定要删除当前记录吗？删除后将不能恢复!!');">删除</a>
                               |&nbsp; <a href='PayUpdate.aspx?id=<%# Eval("ID") %>'>修改</a>
                                </td>
							</tr>
						</ItemTemplate>
					</asp:Repeater>
                    <div>
                    <asp:DropDownList ID="DropDownupdate" runat="server">
                        <asp:ListItem Value="0" Text="普通卡"></asp:ListItem>
                        <asp:ListItem Value="1" Text="新手卡"></asp:ListItem>
                    </asp:DropDownList>
                    <input type="button" value="批量修改" onclick="return AllUpdate()" class="put" />
	...
	<script type="text/javascript" language="javascript"> 
	    ...
	    function AllUpdate() {
	        var selectnewlist = new Array();
	        var selectedCheck = document.getElementsByName("cbxCheck");
	        var ddl = document.getElementById("DropDownupdate")
	        ...
	        for (var i = 0; i < selectedCheck.length; i++) {
	            if (selectedCheck[i].checked) {
	                selectnewlist.unshift(selectedCheck[i].value);

	            }
	        }
	        if (selectnewlist.length <= 0) {
	            alert("至少选择一条记录");
	            return false;
	        }
	        if (confirm("您确定要修改所选的记录吗？")) {
	            alert(location.href + '&batUpdate=batchUpdate&uid=' + selectnewlist.toString());

	            document.location.href = location.href + '&batUpdate=batchUpdate&Value=' + Value + '&uid=' + selectnewlist.toString();
	        }
	        return true;
	    }
	    function NoUseDel() {
