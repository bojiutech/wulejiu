<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MobilePayDiamond.aspx.cs" Inherits="HN.Admin.Admin.Pay.MobilePayDiamond" %>
...
<body>
    ...
    <table width="100%" border="1" align="center" cellpadding="5" cellspacing="0" bordercolor="A4B6D7"
        ...
        <caption>
            <font color='red'><strong>手游充值钻石管理</strong></font></caption>
        <tr>
            <th>
                ID
            </th>
        ...
        <asp:Repeater runat="server" ID="rptDataList">
            <ItemTemplate>
                <tr>
                    ...
                    <td align="center">
                        <a href='MobilePayDiamondConfig.aspx?GameType=<%# Eval("ID") %>'>
                            <%# Eval("GameType").ToString()%></a>
                    </td>
                    <td align="center">
                        <a href="MobilePayDiamond.aspx?action=edit&params=<%# Eval("ID") %>&reurl=<%=Server.UrlEncode(Request.Url.AbsoluteUri) %>">
                            修改</a>&nbsp;&nbsp; <a href="MobilePayDiamond.aspx?action=del&params=<%# Eval("ID") %>&reurl=<%=Server.UrlEncode(Request.Url.AbsoluteUri) %>"
