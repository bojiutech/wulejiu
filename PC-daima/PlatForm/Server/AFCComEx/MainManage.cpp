...
EXT_CLASS unsigned char CUnZipRes::m_bPassWord[MAX_PASSWORD_LENGTH];
...
bool CBaseMainManage::Start()
{
	...
	//调用接口
	if (OnStart()==false) throw new CAFCException(TEXT("CBaseMainManage::Start OnStart 函数错误"),0x41F);
...
bool CBaseMainManage::WindowTimerMessage(UINT uTimerID)
{
	...
	if(7==uTimerID)
	{
		OutputDebugString("ycl::7==uTimerID");
		OnUpdateRoomInfo();
		return true;
	}
	else
...
void CBaseMainManage::OnUpdateRoomInfo()
{
