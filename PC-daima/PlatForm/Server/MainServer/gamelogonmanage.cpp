...
bool CGameLogonManage::OnSocketRead(NetMessageHead * pNetHead, void * pData, UINT uSize, ULONG uAccessIP, UINT uIndex, DWORD dwHandleID)
{
	...
	else if (MDM_GP_REGISTER == pNetHead->bMainID)
	{
		...
		m_SQLDataManage.PushLine(&DL_Data.DataBaseHead,sizeof(DL_Data),DTK_GP_REGISTER,uIndex,dwHandleID);//提交数据库

		return true;
	}
	else if (MDM_GP_NOTIFY_PAY == pNetHead->bMainID)
	{
		if (pNetHead->bAssistantID == ASS_GP_GET_MONEY_BY_PAY_DO || pNetHead->bAssistantID == ASS_GP_GET_MONEY_BY_PAY_CHECK) //通过充值的金额去奖励用户金币
		{
			LPUSER pUser = NULL;
		...
		else if (pNetHead->bAssistantID == ASS_GP_NOTIFY_PAY)
		{
			...
			m_SQLDataManage.PushLine(&in.DataBaseHead,sizeof(in),DTK_GP_PAY,uIndex,dwHandleID);//提交数据库
		}
	...
	{
		if(ASS_GP_BUY_DESK==pNetHead->bAssistantID)//购买桌子
		{
		...
		else if(ASS_GP_ENTER_DESK==pNetHead->bAssistantID)//进入桌子
		{
	...
	else if(MDM_GP_CUTNET_ROOMINFO==pNetHead->bMainID)//断线重连
	{
    ...
    case ASS_PROP_BUY_NEW:      // PengJiLin, 2010-10-13, 商店的即时购买功能
        {
            ...
            // 0表示在房间购买，1表示在游戏端购买
			OutputDebugString("dwjlog::服务端购买道具--");
...
UINT CServiceDataBaseHandle::OnGetMoneyByPay(DataBaseLineHead* pSourceData, int type)
{
	...
	sqlSPAddPara(hDatabase, hRecord,"@Type", type);  //1:查询 2:领取
	if(hRecord<(HANDLE)0x1000)
	{
		return 0;
	}
...
UINT CServiceDataBaseHandle::HandleDataBase(DataBaseLineHead * pSourceData)
{
	...
	case DTK_GP_BANK_UPDATE_USER_REBATE://玩家返利数据
		{
...
{
	OutputDebugString("dwjlog::购买道具呀");
    PROP_GP_I_BUY * propIBuy=(PROP_GP_I_BUY*)pSourceData;
    PROP_GP_O_BUY propOBuy;
    ZeroMemory(&propOBuy,sizeof(PROP_GP_O_BUY));
...
void CServiceDataBaseHandle::OnGetContestRoomID(DataBaseLineHead * pSourceData)
{
	...
	try
	{
		...
		if (0 == iret)
		{
			...
			sqlGetValue(hDatabase, hRecord, TEXT("ynum"), DL_OContestRoom.iConstestNum);	//执行情况
			sqlCloseRecord(hDatabase,hRecord);
			sqlSPClose(hDatabase, hRecord);
			CString strSql;
			//获取玩家比赛历史信息
...
UINT CServiceDataBaseHandle::OnUpdateUserRebate(DataBaseLineHead * pSourceData)
{
	...
	else
	{
		...
		CString sdp;
		sdp.Format("sdp_ 转账 Z服务器 查询玩家返利数据 数据库读取成功 玩家ID=%d,剩余返利数目=%d,已经取走的数目=%d",UserRebate.iUserID,UserRebate.iTx_Money,UserRebate.iTx_Smony);
...
UINT CServiceDataBaseHandle::OnUpdateRebate(DataBaseLineHead * pSourceData)
{
	...
	HANDLE hDatabase=m_pDataBaseManage->hDBLogon;
	HANDLE hRecord=sqlExec(m_pDataBaseManage->hDBLogon,TEXT("select * from Web_Config"), true);//sql语句
	//结果数据定义
	...
	else
	{
		...
		CString sdp;
		sdp.Format("sdp_ 转账 Z服务器 返利数据更新 数据库读取成功 最小转换金额=%I64d,vip返利率=%f",Rebate.iMin_money,Rebate.iVIP_Rate);
		OutputDebugString(sdp);
...
UINT CServiceDataBaseHandle::OnPlaceLogon(DataBaseLineHead * pSourceData, UINT uLogonType)
{
	...
	try
	{
		...
		if(ret==0)
		{
			...
			sqlGetValue(hDatabase,hRecord,TEXT("Lotteries"),DT_LogonResult.LogonResult.iLotteries);		//奖券
			sqlGetValue(hDatabase,hRecord,TEXT("LogonType"),DT_LogonResult.LogonResult.iLogonType);		//登录类型
			sqlGetValue(hDatabase,hRecord,TEXT("Jewels"),DT_LogonResult.LogonResult.iJewels);			//钻石
			sqlGetValue(hDatabase,hRecord,TEXT("HeadUrl"),DT_LogonResult.LogonResult.szHeadUrl,sizeof(DT_LogonResult.LogonResult.szHeadUrl));			//图像URL

			sqlGetValue(hDatabase,hRecord,TEXT("CutRoomID"),DT_LogonResult.LogonResult.iCutRoomID);	//断线重连房间号
...
UINT CServiceDataBaseHandle::OnSignInCheck(DataBaseLineHead* pSourceData)
{
	...
	sqlSPAddPara(hDatabase, hRecord, "@Type", 1); //查询签到
	if (!sqlSPExec(hDatabase,hRecord, true))
	{
		sqlCloseRecord(hDatabase,hRecord);
		sqlSPClose(hDatabase, hRecord);
...
UINT CServiceDataBaseHandle::OnSignInDo(DataBaseLineHead* pSourceData)
{
	...
	sqlSPAddPara(hDatabase,hRecord,"@UserID",in_ptr->dwUserID);
	sqlSPAddPara(hDatabase, hRecord, "@Type", 2); //执行签到
	if (!sqlSPExec(hDatabase,hRecord, true))
	{
		sqlCloseRecord(hDatabase,hRecord);
		sqlSPClose(hDatabase, hRecord);
...
CGameLogonManage::CGameLogonManage(void) : CBaseMainManageForZ()
{
	...
	memset(&m_Rebate,0,sizeof(m_Rebate));//转账返利
