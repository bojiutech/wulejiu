...
/*------------------------------------------------------------------------------------------------------*/
//文件名字定义
#define GAMENAME						TEXT("安庆麻将") 
#define NAME_ID						    20116004  		// 名字 ID
/*------------------------------------------------------------------------------------------------------*/
...
#define HUA_CARD_NUM			16				//花牌张数
#define MEN_CARD_NUM			34				//门牌张数
#define MAX_HUPAI_TYPE			21				//糊牌类型
#define MAX_HAND_PAI			14				//手牌数
/*------------------------------------------------------------------------------------------------------*/
///糊牌检测临时数据
...
enum MJTHINGS
{
    ...
    THING_HAVE_ACTION		,                           
	THING_CAN_TING			,
	THING_CALCULATE_BOARD=164,			//大结算
	THING_LAST_ROUNDS		,			//剩余局数
	THING_MAX		                //最大事件id
};                              
...
{
	INT bBestFire;//点炮最多玩家
	INT bWinner;//最大赢家
	INT iZimo[PLAY_COUNT];//自摸次数
	INT iJiePao[PLAY_COUNT];//接炮次数
	INT iDianPao[PLAY_COUNT];//点炮次数
	INT iAnGan[PLAY_COUNT];//暗杠次数
	INT iMingGan[PLAY_COUNT];//明杠次数
	long long i64WinMoney[PLAY_COUNT];//输赢金币
	CalculateBoard() {
		init();
	}
	void init() {
