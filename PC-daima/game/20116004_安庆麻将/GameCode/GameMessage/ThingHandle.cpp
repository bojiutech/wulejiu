...
bool ThingHandle::GetGameStaton(BYTE bDeskStation, bool bWatchUser,BYTE GameStation)
{
	...
	GameStationData.byGameStation = pDesk->m_byGameStation;		 //游戏状态
	//断线重连取消托管
...
void ThingHandle::CheckNextAction()
{
	...
	switch(pDesk->sGameData.m_byMaxAction)//确定了下一步的响应事件：吃，碰，杠，胡
	{
...
bool ThingHandle::IsCanHandle()
{
	...
	{//考虑抢杠胡等操作
		...
		if(max == ACTION_NO)
		{
			pDesk->sGameData.m_byMaxAction = ACTION_BU_GANG;//继续执行完补杠
			return true;
		}
		//查找是否有玩家响应
